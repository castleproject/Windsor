...
namespace CastleTests
{
	...
	public class LazyComponentsTestCase : AbstractContainerTestCase
	{
		...
		public void Can_resolve_lazy_component_requiring_arguments_inline()
		{
			...
			var a = new A();
			var arguments = new Arguments().InsertTyped(a);
			var missingArguments = Container.Resolve<Lazy<B>>();
			var hasArguments = Container.Resolve<Lazy<B>>(new Arguments().InsertProperties(new { arguments }));
		...
		public void Can_resolve_lazy_component_with_override()
		{
			...
			var lazyA = Container.Resolve<Lazy<A>>(new Arguments().InsertProperties(new { overrideComponentName = "2" }));
