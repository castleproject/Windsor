...
namespace Castle.Facilities.WcfIntegration.Internal
{
	...
	public abstract class AsyncResult : IAsyncResult
	{
		...
		/// <summary>
		/// Initializes a new <see cref="AsyncResult"/>
		/// </summary>
		...
		public WaitHandle AsyncWaitHandle
		{
			get
			{
				...
				if (waitEvent == null)
				{
					Interlocked.CompareExchange(ref waitEvent, new ManualResetEvent(isCompleted != 0), (object)null);
				}
		...
		public Timeout TimeoutAfter(int interval)
		{
			lock (this)
			{
				if (timeout != null)
				{
					throw new InvalidOperationException("A timeout has already been established");
				}
		...
		public class Timeout
		{
			...
			public Timeout(AsyncResult result, int interval)
			{
				this.result = result;
			...
			/// </summary>
			/// <returns></returns>
			public AsyncResult MilliSeconds()
			{
